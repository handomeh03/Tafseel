...
import { useStoreStats } from "@/features/dashboard/hooks/useStoreStats";
...
export default function AdminDashboardPage() {
  ...
  const { storeStats, isLoading: isStoresLoading } = useStoreStats();

  const isLoading = isOrdersLoading || isProductsLoading || isStoresLoading;
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          أهلاً {user?.name || "بك"}، إليك نظرة عامة على أداء منصة تفصيل اليوم
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <StatCard
              icon={ClipboardList}
              label="طلبات المتاجر المعلقة"
              value={storeStats?.requestCounts?.PENDING ?? 0}
              subLabel={`${storeStats?.totalRequests ?? 0} طلب إجمالي`}
              accent="amber"
            />
            <StatCard
              icon={Package}
              ...
              subLabel={`${productStats?.availableProducts ?? 0} متاح`}
              accent="violet"
            />
            <StatCard
              icon={ShoppingCart}
